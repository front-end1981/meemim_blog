<?php
/**
 * Plugin Name: Social Count Custom
 */
class SocialCount {
	...
	function getEndpoints () {
		$url = (isset($this->url) ? $this->url : FALSE);
		if(!$url) die();
...
//						}),
	...
	function apiCall($method, $url, $params = array()) {
		...
		if ($this->api) {
			...
			curl_setopt($this->api, CURLOPT_POST, false);
			curl_setopt($this->api, CURLOPT_POSTFIELDS, NULL);
			if(isset($params['headers'])) curl_setopt($this->api, CURLOPT_HTTPHEADER, $params['headers']);

			if ($method == 'GET') {
				curl_setopt($this->api, CURLOPT_HTTPGET, true);

			} elseif ($method == 'POST') {
				curl_setopt($this->api, CURLOPT_POST, true);
				if(isset($params['payload'])) curl_setopt($this->api, CURLOPT_POSTFIELDS, $params['payload']);

			} elseif ($method == 'PUT') {
				curl_setopt($this->api, CURLOPT_CUSTOMREQUEST, 'PUT');

			} elseif ($method == 'DELETE') {
				curl_setopt($this->api, CURLOPT_CUSTOMREQUEST, 'DELETE');
			...
			curl_setopt($this->api, CURLOPT_URL, rtrim($url, '?&'));
			curl_setopt($this->api, CURLOPT_TIMEOUT, 5);
			curl_setopt($this->api, CURLOPT_HEADER, false);
			curl_setopt($this->api, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($this->api, CURLOPT_SSL_VERIFYPEER, false);
			curl_setopt($this->api, CURLOPT_FOLLOWLOCATION, true);

			if ($raw = curl_exec($this->api)) {
