...
class SimpleLinkListWidget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		for ($i = 1; $i <= $amount; $i++) {
            ...
            $item_descriptions[$i-1] = $instance['item_description'.$i];
		...
		if($reverse){
            ...
            $item_descriptions = array_reverse($item_descriptions);
	...
	public function update( $new_instance, $old_instance) {
		...
		if($order){
			foreach ($order as $i => $item_num) {
				...
				$instance['item_description'.($i+1)] = empty($new_instance['item_description'.$item_num]) ? '' : strip_tags($new_instance['item_description'.$item_num]);
	...
	public function form( $instance ) {
		...
		for ($i = 1; $i <= $amount; $i++) {
            ...
            $item_descriptions[$i] = empty($instance['item_description'.$i]) ? '' : $instance['item_description'.$i];
...
?>
		...
		<?php foreach ($items as $num => $item) :
            ...
            $item_description = esc_attr($item_descriptions[$num]);
		...
		?>
			...
			<div id="<?php echo $this->get_field_id($num); ?>" class="list-item">
				...
				<div class="sllw-edit-item">
                    ...
                    <label for="<?php echo $this->get_field_id('item_description'.$num); ?>"><?php echo __("Description:"); ?></label>
                    <input class="widefat" id="<?php echo $this->get_field_id('item_description'.$num); ?>" name="<?php echo $this->get_field_name('item_description'.$num); ?>" type="text" value="<?php echo $item_description; ?>" />
